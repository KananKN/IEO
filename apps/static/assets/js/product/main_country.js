...
function sweetAlertDel(id) {
    swal({
        ...
        if (result.dismiss !== "cancel") {
            post("product/deleteCountry", { id: id });
        }
    });
}
...
function func_modal(mode, data) {
    ...
    if (mode == 'add') {
        ...
        $('[name="id"]').val(data.id);
        $('[name="name_country"]').val(data.name);
