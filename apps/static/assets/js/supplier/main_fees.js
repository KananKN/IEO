...
function func_modal(mode, data) {
    ...
    if (mode == 'add') {
        ...
        $('.modal-title').html('Edit Fees');
        $('.c_add').hide()
        $('.c_edit').show()

        $('[name="id-update"]').val(data.data_fee.id);
        $('[name="name_fees"]').val(data.name);
...
function sweetAlertDel(id) {
    swal({
        ...
        if (result.dismiss !== "cancel") {
            post("/supplier/delete_fees", { id: id });
        }
    });
}