...
function sweetAlertDel(id, x) {
    ...
    swal({
        ...
        fetch(`/account/delete_currencies/${id}`, {
            method: "DELETE",
            headers: {
        ...
        $.ajax({
            ...
            error(err) {
                ...
                Swal.fire({
                    ...
                    text: err.responseJSON?.message || 'ไม่สามารถบันทึกข้อมูลได้',
...
$(document).ready(function () {
    ...
    $('#requester_user_id').on('change', function () {
        ...
        if (!userId) {
            ...
            $('#phone').val('');
            $('#email').val('');
            $('#modal-user-tbody').hide();

            $('#modal-full-name').text('-');
            $('#modal-bank-account').text('-');
            $('#modal-bank-name').text('-');
            return;
        }
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (!hasData) {
                    // ❌ ไม่มีข้อมูล → ซ่อน tbody
                    $('#modal-user-tbody').hide();
                    return;
                }

                // ✅ มีข้อมูล → แสดง tbody
                $('#modal-user-tbody').show();
...
//     });
    ...
    //$('#staffTotal').text(total.toFixed(2));
