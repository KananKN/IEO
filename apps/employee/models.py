...
class EmployeeModel(db.Model):
    ...
    files_employee = db.relationship(
        "FileEmployeeModel", backref="files_employee", cascade="all, delete", lazy=True )
    ...
    coordinator = db.relationship(
        "CoordinatorModel", backref="employee", cascade="all, delete", lazy=True ) 
...
@dataclass
class FileEmployeeModel(db.Model):
    __tablename__ = 'files_employee'
    id :int
    employee_id :int
    ...
    file_type  = db.Column(db.Integer, comment='1:employee')
...
@dataclass        
class ProductEmployerAssociation(db.Model):
    __tablename__ = 'product_employer_association'
    ...
    product = db.relationship('ProductForSalesModel', backref='employee_links')
    employee = db.relationship('EmployeeModel', backref='product_links')    
